...
sys.path.append(
    os.path.join(os.path.dirname(os.path.realpath(__file__)), "libevents/libs/python")
)
...
def get_sw_releases():
    ...
    if (
        download_file_maybe(
            releases_json, "https://api.github.com/repos/PX4/Firmware/releases"
        )
        > 0
    ):
...
def get_total_flight_time(ulog):
    ...
    """
    if (
        "LND_FLIGHT_T_HI" in ulog.initial_parameters
        and "LND_FLIGHT_T_LO" in ulog.initial_parameters
    ):
...
def get_drone_name(drotek_uuid):
    with open(
        os.path.dirname(os.path.realpath(__file__)) + "/../../data/alias.yml"
    ) as f:
        alias_db = yaml.safe_load(f)

        if drotek_uuid in alias_db["alias"]:
            return alias_db["alias"][drotek_uuid]
        else:
            return ""
