...
def get_heading_html(
    ulog, px4_ulog, db_data, link_to_3d_page, additional_links=None, title_suffix=""
):
    """
    ...
    if link_to_3d_page is not None and any(
        elem.name == "vehicle_gps_position" for elem in ulog.data_list
    ):
        link_to_3d = (
    ...
    title_html = (
        "<table width='100%'><tr><td><h3>"
        + sys_name
        + px4_ulog.get_mav_type()
        + title_suffix
...
def get_info_table_html(ulog, px4_ulog, db_data, vehicle_data, vtol_states):
    ...
    if airframe_name_tuple is not None:
        ...
        if len(airframe_name) == 0:
            ...
            table_text_left.append(
                ("Airframe", airframe_name + " <small>(" + airframe_id + ")</small>")
            )
    ...
    if (
        "sys_os_name" in ulog.msg_info_dict
        and "sys_os_ver_release" in ulog.msg_info_dict
    ):
    ...
    try:
        ...
        if len(indices[0]) > 0:
            ...
            utctimestamp = datetime.datetime.utcfromtimestamp(
                logging_start_time + utc_offset_min * 60
...
Log timezone: {}
<br />
SDLOG_UTC_OFFSET: {}""".format(
                utctimestamp.strftime("%d-%m-%Y %H:%M"), utc_offset_min
            )
            tooltip = (
    ...
    if len(dropout_durations) > 0:
        ...
        table_text_left.append(
            (
                "Dropouts",
                "{:} ({:} s)".format(len(dropout_durations), total_duration_str),
            )
        )
    ...
    if flight_time_s is not None:
        ...
        flight_time_str = ""
        if days > 0:
            flight_time_str += "{:d} days ".format(days)
        if h > 0:
            flight_time_str += "{:d} hours ".format(h)
        if m > 0:
            flight_time_str += "{:d} minutes ".format(m)
    ...
    if (
        "sys_uuid" in ulog.msg_info_dict
        and sys_hardware != "SITL"
        and sys_hardware != "PX4_SITL"
    ):
    ...
    try:
        ...
        pos_xyz_valid = np.multiply(
            local_pos.data["xy_valid"], local_pos.data["z_valid"]
        )
        local_vel_valid_indices = np.argwhere(
            np.multiply(local_pos.data["v_xy_valid"], local_pos.data["v_z_valid"]) > 0
        )
        ...
        if len(vel_x) > 0:
            ...
            table_text_right.append(
                ("Max Speed Horizontal", "{:.1f} km/h".format(max_h_speed * 3.6))
            )
            table_text_right.append(
                ("Max Speed Up", "{:.1f} km/h".format(np.amax(-vel_z) * 3.6))
            )
            table_text_right.append(
                ("Max Speed Down", "{:.1f} km/h".format(-np.amin(-vel_z) * 3.6))
            )
        ...
        if len(roll) > 0:
            ...
            tilt_angle = (
                np.arccos(np.multiply(np.cos(pitch), np.cos(roll))) * 180 / np.pi
            )
            table_text_right.append(
        ...
        if len(rollspeed) > 0:
            max_rot_speed = np.amax(
                np.sqrt(
                    np.square(rollspeed) + np.square(pitchspeed) + np.square(yawspeed)
                )
            )
            table_text_right.append(
                (
    ...
    def generate_html_table(rows_list, tooltip=None, max_width=None):
        ...
        if tooltip is None:
            ...
            tooltip = (
                ...
                + 'data-delay=\'{"show": 1000, "hide": 100}\' title="'
                + tooltip
                + '" '
            )
        table = "<table " + tooltip
...
def get_error_labels_html():
    ...
    for err_id, err_label in error_labels_table.items():
        error_label_select += '<option data-id="{:d}">{:s}</option>'.format(
            err_id, err_label
        )
...
def get_changed_parameters(ulog, plot_width):
    ...
    for param_name in sorted(initial_parameters):
        ...
        if airframe_defaults is not None:
            ...
            is_airframe_default = (
                abs(float(airframe_default) - float(param_value)) < 0.00001
            )
        ...
        try:
            if param_name in default_params:
                ...
                if default_param["type"] == "FLOAT":
                    is_default = (
                        abs(float(system_default) - float(param_value)) < 0.00001
                    )
                    if "decimal" in default_param:
                        param_value = round(param_value, int(default_param["decimal"]))
                        airframe_default = round(
                            float(airframe_default), int(default_param["decimal"])
    ...
    columns = [
        TableColumn(
            field="names", title="Name", width=int(plot_width * 0.2), sortable=False
        ),
        TableColumn(
            field="values",
            title="Value",
        ...
        TableColumn(
            field="mins", title="Min", width=int(plot_width * 0.075), sortable=False
        ),
        TableColumn(
            field="maxs", title="Max", width=int(plot_width * 0.075), sortable=False
        ),
        TableColumn(
            field="descriptions",
            title="Description",
    ...
    div = Div(
        text="""<b>Non-default Parameters</b> (except RC and sensor calibration)""",
...
def get_logged_messages(ulog, plot_width):
    ...
    for m in logged_messages:
        ...
        if m.message[-1] == "\t":
            continue
        messages.append(
            (m.timestamp, time_str(m.timestamp), m.log_level_str(), m.message)
        )
    ...
    log_data = dict(times=log_times_str, levels=log_levels, messages=log_messages)
    source = ColumnDataSource(log_data)
    columns = [
        TableColumn(
            field="times", title="Time", width=int(plot_width * 0.15), sortable=False
        ),
        TableColumn(
            field="levels", title="Level", width=int(plot_width * 0.1), sortable=False
        ),
        TableColumn(
            field="messages",
            title="Message",
