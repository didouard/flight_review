...
def generate_plots(
    ...
    if any(
        elem.name == "vehicle_air_data" or elem.name == "vehicle_magnetometer"
        for elem in data
    ):
    ...
    data_plot.add_graph(
        [lambda data: ("alt", data["alt"] * 0.001)], colors8[0:1], ["GPS Altitude"]
    )
    ...
    data_plot.add_graph(
        [lambda data: ("thrust", data["control[3]"] * 100)],
        colors8[6:7],
        ["Thrust [0, 100]"],
    )
    ...
    if any(elem.name == "manual_control_setpoint" for elem in data):
        ...
        data_plot.add_graph(
            ["y", "x", "r", "z", "aux1", "aux2"],
            colors8[0:6],
            ["Y / Roll", "X / Pitch", "Yaw", "Throttle [0, 1]", "Aux1", "Aux2"],
        )
        ...
        if data_plot.dataset:
            ...
            if max_channels < num_rc_channels:
                num_rc_channels = max_channels
        legends = []
    ...
    data_plot.add_graph(
        ["control[0]", "control[1]", "control[2]"], colors3, ["Roll", "Pitch", "Yaw"]
    )
    ...
    data_plot.add_graph(
        ["vibe[2]"], colors3[2:3], ["Delta Velocity Vibration Level [m/s]"]
    )
    data_plot.add_horizontal_background_boxes(["green", "orange", "red"], [0.02, 0.04])
    ...
    if add_virtual_fifo_topic_data(ulog, "sensor_accel_fifo"):
        ...
        data_plot = DataPlot(
            ...
            y_axis_label="[us]",
            title="Sampling Regularity of Sensor Data (FIFO)",
            plot_height="small",
            changed_params=changed_params,
            x_range=x_range,
        )
    ...
    if add_virtual_fifo_topic_data(ulog, "sensor_gyro_fifo"):
        ...
        data_plot = DataPlotSpec(
            ...
            "sensor_gyro_fifo_virtual",
            y_axis_label="[Hz]",
            title="Gyro Power Spectral Density (FIFO)",
            plot_height="normal",
            x_range=x_range,
        )
        data_plot.add_graph(["x", "y", "z"], ["X", "Y", "Z"])
    ...
    data_plot.add_graph(
        ["eph", "epv", "satellites_used", "fix_type"],
        colors8[::2],
        [
    ...
    data_plot.change_dataset("actuator_controls_0")
    data_plot.add_graph(
        [lambda data: ("thrust", data["control[3]"])], colors3[1:2], ["Thrust"]
    )
    if is_vtol:
        data_plot.change_dataset("actuator_controls_1")
        data_plot.add_graph(
            [lambda data: ("thrust", data["control[3]"])],
            colors3[2:3],
            ["Thrust (Fixed-wing)"],
        )
    if data_plot.finalize() is not None:
    ...
    data_plot.add_graph(
        [lambda data: ("rssi", data["rssi"] / 100), "rc_lost"],
        colors3[0:2],
        ["RSSI [0, 1]", "RC Lost (Indicator)"],
    )
