...
if False:
    ...
    try:
        ...
        error_message = (
            "A parsing error occured when trying to read the file - "
    ...
    if error_message == "":
        ...
        try:
            ...
            # cur = con.cursor()
            # cur.execute(
            #     "select Description, Feedback, Type, WindSpeed, Rating, VideoUrl, "
            #     "ErrorLabels from Logs where Id = ?",
            #     [log_id],
            # )
            ...
            #     db_data.error_labels = (
            #         sorted([int(x) for x in db_tuple[6].split(",") if len(x) > 0])
            #         if db_tuple[6]
            #         else []
            #     )
            ...
            if "sys_uuid" in ulog.msg_info_dict:
                ...
                # cur.execute(
                #     "select LatestLogId, Name, FlightTime "
        ...
        #         plots = get_pid_analysis_plots(
        #             ulog, px4_ulog, db_data, link_to_main_plots
        #         )
        ...
        if True:
            ...
            ] = get_mapbox_api_access_token()
            curdoc().template_variables["is_plot_page"] = True
            curdoc().template_variables["log_id"] = log_id
            flight_modes = [
                {
                    "name": "Manual",
                    "color": HTML_color_to_RGB(flight_modes_table[0][1]),
                },
                {
                    "name": "Altitude Control",
                    "color": HTML_color_to_RGB(flight_modes_table[1][1]),
                },
                {
                    "name": "Position Control",
                    "color": HTML_color_to_RGB(flight_modes_table[2][1]),
                },
                {"name": "Acro", "color": HTML_color_to_RGB(flight_modes_table[10][1])},
                {
                    "name": "Stabilized",
                    "color": HTML_color_to_RGB(flight_modes_table[15][1]),
                },
                {
                    "name": "Offboard",
                    "color": HTML_color_to_RGB(flight_modes_table[14][1]),
                },
                {
                    "name": "Rattitude",
                    "color": HTML_color_to_RGB(flight_modes_table[16][1]),
                },
                {
        ...
        div = Div(
            text="<h3>Error</h3><p>" + error_message + "</p>",
