...
def get_arguments():
    ...
    parser.add_argument(
        "--download-api",
        type=str,
        default="https://review.px4.io/download",
...
def main():
    ...
    if args.print_entries:
        ...
        if args.mav_type is not None:
            ...
            db_entries_list = [
                entry for entry in db_entries_list if entry["mav_type"].lower() in mav
            ]
        ...
        if args.rating is not None:
            ...
            db_entries_list = [
                entry for entry in db_entries_list if entry["rating"].lower() in rate
            ]
        ...
        if args.log_id is not None:
            arg_log_ids_without_dashes = [
                log_id.replace("-", "") for log_id in args.log_id
            ]
            db_entries_list = [
                entry
                for entry in db_entries_list
        ...
        if args.vehicle_name is not None:
            db_entries_list = [
                entry
                for entry in db_entries_list
                if entry["vehicle_name"] == args.vehicle_name
            ]
        ...
        if args.airframe_name is not None:
            db_entries_list = [
                entry
                for entry in db_entries_list
                if entry["airframe_name"] == args.airframe_name
            ]
        ...
        if args.airframe_type is not None:
            db_entries_list = [
                entry
                for entry in db_entries_list
                if entry["airframe_type"] == args.airframe_type
            ]
                ...
                try:
                    if args.overwrite or entry_id not in logids:
                        ...
                        file_path = os.path.join(
                            args.download_folder, entry_id + ".ulg"
                        )

                        print("downloading {:}/{:} ({:})".format(i + 1, n_en, entry_id))
                        request = requests.get(
                            url=args.download_api + "?log=" + entry_id, stream=True
                        )
        ...
        print(
            "{:} logs downloaded to {:}, {:} logs skipped (already downloaded)".format(
