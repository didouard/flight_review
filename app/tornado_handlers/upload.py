...
from .common import (
    get_jinja_env,
    CustomHTTPError,
    generate_db_data_from_log_file,
...
def update_vehicle_db_entry(cur, ulog, log_id, vehicle_name):
    ...
    if "sys_uuid" in ulog.msg_info_dict:
        ...
        if vehicle_name == "":
            cur.execute(
                "select Name " "from Vehicle where UUID = ?", [vehicle_data.uuid]
            )
    ...
    def post(self, *args, **kwargs):
        ...
        if self.multipart_streamer:
            try:
                ...
                if ulog is not None and "sys_uuid" in ulog.msg_info_dict:
                    sys_uuid = escape(ulog.msg_info_dict["sys_uuid"])

                drotek_uuid = get_drotek_uuid(sys_uuid)
                vehicle_name = get_drone_name(drotek_uuid)
                ...
                cur.execute(
                    ...
                    "videoUrl, ErrorLabels, Public, Token, UUID) values "
                    "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [
                        log_id,
                        title,
                ...
                if ulog is not None:
                    ...
                    if "ver_sw_branch" in ulog.msg_info_dict:
                        branch_info = (
                            " (branch: " + ulog.msg_info_dict["ver_sw_branch"] + ")"
                        )
